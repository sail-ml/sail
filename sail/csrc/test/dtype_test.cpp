...
inline std::vector<Dtype> all_dtypes = {
    Dtype::sBool,   Dtype::sInt8,    Dtype::sUInt8,  Dtype::sInt16,
    Dtype::sUInt16, Dtype::sInt32,   Dtype::sUInt32, Dtype::sInt64,
    Dtype::sUInt64, Dtype::sFloat32, Dtype::sFloat64};
inline std::vector<std::string> all_dtype_string = {
...
TEST(DtypeTest, sbool) {
    ...
    ASSERT_THROW(dispatch_fp_int_types(y.get_dtype(),
                                       [&](auto pt) { ASSERT_TRUE(false); },
                                       [&](auto pt2) { ASSERT_TRUE(true); }),
                 DtypeError);
}
TEST(DtypeTest, uint8) {
    auto x = random::uniform(TensorShape({10}), 0, 100);
    auto y = x.cast(Dtype::sUInt8);
    dispatch_fp_int_types(y.get_dtype(), [&](auto pt) { ASSERT_TRUE(false); },
                          [&](auto pt2) { ASSERT_TRUE(true); });
}
...
TEST(DtypeTest, float32) {
    ...
    dispatch_fp_int_types(y.get_dtype(), [&](auto pt) { ASSERT_TRUE(true); },
                          [&](auto pt2) { ASSERT_TRUE(false); });
}
...
TEST(DtypeTest, dispatch_all) {
    for (int i = 0; i < all_dtypes.size(); i++) {
        dispatch_all_types(all_dtypes[i], [&](auto pt) { ASSERT_TRUE(true); });
    }
}
TEST(DtypeTest, dispatch_all_numeric) {
    for (int i = 1; i < all_dtypes.size(); i++) {
        dispatch_all_numeric_types(all_dtypes[i],
                                   [&](auto pt) { ASSERT_TRUE(true); });
    }

    ASSERT_THROW(dispatch_all_numeric_types(Dtype::sBool, [&](auto pt) {}),
                 DtypeError);
}
...
TEST(DtypeTest, Promotion) {
    ...
    ASSERT_EQ(promote_dtype(Dtype::sFloat32, Dtype::sUInt16, true),
              Dtype::sFloat32);
    ASSERT_EQ(promote_dtype(Dtype::sFloat32, Dtype::sUInt8, true),
              Dtype::sFloat32);
    ASSERT_EQ(promote_dtype(Dtype::sFloat32, Dtype::sInt8, true),
              Dtype::sFloat32);
    ASSERT_EQ(promote_dtype(Dtype::sFloat32, Dtype::sInt16, true),
              Dtype::sFloat32);

    ASSERT_EQ(promote_dtype(Dtype::sFloat32, Dtype::sUInt32, true),
              Dtype::sFloat64);
    ASSERT_EQ(promote_dtype(Dtype::sFloat32, Dtype::sInt32, true),
              Dtype::sFloat64);
    ASSERT_EQ(promote_dtype(Dtype::sFloat32, Dtype::sUInt64, true),
              Dtype::sFloat64);
    ASSERT_EQ(promote_dtype(Dtype::sFloat32, Dtype::sInt64, true),
              Dtype::sFloat64);

    ASSERT_EQ(promote_dtype(Dtype::sUInt16, Dtype::sFloat32, true),
              Dtype::sFloat32);
    ASSERT_EQ(promote_dtype(Dtype::sUInt8, Dtype::sFloat32, true),
              Dtype::sFloat32);
    ASSERT_EQ(promote_dtype(Dtype::sInt8, Dtype::sFloat32, true),
              Dtype::sFloat32);
    ASSERT_EQ(promote_dtype(Dtype::sInt16, Dtype::sFloat32, true),
              Dtype::sFloat32);

    ASSERT_EQ(promote_dtype(Dtype::sUInt32, Dtype::sFloat32, true),
              Dtype::sFloat64);
    ASSERT_EQ(promote_dtype(Dtype::sInt32, Dtype::sFloat32, true),
              Dtype::sFloat64);
    ASSERT_EQ(promote_dtype(Dtype::sUInt64, Dtype::sFloat32, true),
              Dtype::sFloat64);
    ASSERT_EQ(promote_dtype(Dtype::sInt64, Dtype::sFloat32, true),
              Dtype::sFloat64);
