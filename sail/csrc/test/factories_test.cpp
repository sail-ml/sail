...
TEST(FactoriesTest, OneHot) {
    ...
    sail::Tensor integer_tensor =
        sail::from_data((void*)arr, Dtype::sInt32, sail::TensorShape({10}));
...
TEST(FactoriesTest, OneHotThrow) {
    ...
    sail::Tensor t =
        sail::from_data((void*)arr, Dtype::sFloat32, sail::TensorShape({10}));

    ASSERT_THROW(sail::one_hot(t, 10), SailCError);
}

TEST(FactoriesTest, Ones) {
    sail::Tensor one_tensor =
        sail::ones(sail::TensorShape({32, 32, 32}), Dtype::sInt32);
...
TEST(FactoriesTest, Full) {
    sail::Tensor one_tensor =
        sail::full((double)10.3, sail::TensorShape({32, 32, 32}));
