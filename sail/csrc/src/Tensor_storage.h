...
class TensorStorage {
   public:
    int arr_numel;
    TensorStorage(){};
    TensorStorage(int nd, void* in_data, int64_t in_offset, Dtype dt,
                  TensorShape st, TensorShape sh) {
        // index = in_index;
        ...
        data = _realloc_align(in_data, _numel(sh), info.alignment,
                              info.dtype_size);
    }
    ...
    TensorSize getShape() { return shape; }

    static TensorStorage createEmpty(int nd, void* in_data, int64_t in_offset,
                                     Dtype dt, TensorSize st, TensorSize sh) {
        TensorStorage sto;
    ...
    std::string getStridetring() {
        ...
        for (long value : strides) {
            stride_string =
                stride_string + std::string("aa, ");  //("%d, ", value);
        }
    ...
    std::vector<TensorSize> step_back() {
        TensorSize new_strides;
    ...
    size_t getDtypeSize() { return GetDtypeSize(dtype); }
    size_t getTotalSize() {
    ...
    int numel() const { return arr_numel; }
   ...
   private:
    static int _numel(TensorSize _shape) {
        auto size = 1;
