...
RETURN_OBJECT ops_expand_dims(PyObject* self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "Oi", &t1, &dim)) {
        PyErr_SetString(PyExc_TypeError, "Inputs should be a sail tensor and an integer");
        return NULL;
    }

    if (dim < -1 || dim > t1->tensor.storage.ndim) {
        PyErr_SetString(PyExc_ValueError, ("dim must be in the range of [-1, %s]", t1->tensor.storage.ndim));
        return NULL;
    }
